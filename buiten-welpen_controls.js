...
"buttonBox": [
        {
            ...
            "button": [
                ...
                },
                {
                    ...
                    "id": "hideTbButton",
                    "index": 1,
                    "onclick": "function(){ if (!this.allowClick) return; this.allowClick = false; var tb = this.get('toolbar'); tb.style = 'transform-origin: 50% 100% 0px; transform: translate(0px,0px) translateZ(10000px) perspective( 800px ) rotateX( 90deg ); background: rgba(0,0,0,0.5); transition: transform 500ms;', tb.updateStyles(); tb.tween({'delay': 0.5, 'time': 0.01, 'onComplete': function(a){ a.setVisible(false); a.get('showTbButton').allowClick = true; } }); var tbb = this.get('showTbButton');  tbb.setVisible(true); tbb.style ='transform-origin: 50% 100% 0px; transform: translate(0px,0px) translateZ(10000px) perspective( 800px ) rotateX( 90deg ); transition: transform 500ms;'; tbb.updateStyles();  tbb.tween({'delay': 0.01, 'time': 0.01, 'onComplete': function(a){ a.style ='transform-origin: 50% 100% 0px; transform: translate(0px,0px) translateZ(10000px) perspective( 800px ) rotateX( 0deg ); transition: transform 500ms;'; a.updateStyles(); } });var btn = this.get('galleryBtnPrev'); if (btn&&this.viewer.hasGallery){ btn.style = 'transform-origin: -30% 50% 0px; transform: translate(0px,0px) translateZ(10000px) perspective( 800px ) rotateY( 90deg ); transition: transform 500ms;'; btn.updateStyles(); btn.tween({ 'delay': 0.5, 'time' : 0.01, 'onComplete' : function(a){ a.setVisible(false); } }); }var btn = this.get('galleryBtnNext'); if (btn&&this.viewer.hasGallery){ btn.style = 'transform-origin: 130% 50% 0px; transform: translate(0px,0px) translateZ(10000px) perspective( 800px ) rotateY( 90deg ); transition: transform 500ms;'; btn.updateStyles(); btn.tween({ 'delay': 0.5, 'time' : 0.01, 'onComplete' : function(a){ a.setVisible(false); } }); }}",
                ...
                },
                {
                    ...
                    "priority": 1,
                    "skin": "shadow(3,0,0,rgba(0,0,0,1));copy(defaultSkin,128,256,64,64,0,0,28,28);",
                    "xoff": 0,
                    "yoff": 0
                },
                {
